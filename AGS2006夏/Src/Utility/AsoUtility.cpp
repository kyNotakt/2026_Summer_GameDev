...
void AsoUtility::DrawArc3D(const VECTOR& center, float radius, float startAngle, float endAngle, float rotY, int div, int color)
{
    float step = (endAngle - startAngle) / div;
    ...
    for (int i = 0; i < div; i++)
    {
        ...
        // ボスの向き(Y回転)を加算
        float angle0 = a0 + rotY;
        float angle1 = a1 + rotY;
    ...
    // 扇形の両端を中心へ結ぶ
    VECTOR edge0 = {
        center.x + cosf(startAngle + rotY) * radius,
        center.y,
        center.z + sinf(startAngle + rotY) * radius
    };
    VECTOR edge1 = {
        center.x + cosf(endAngle + rotY) * radius,
        center.y,
        center.z + sinf(endAngle + rotY) * radius
    };

    DrawLine3D(center, edge0, color);
    DrawLine3D(center, edge1, color);
}
